...
public:
    ...
    bool getOption(int level, int option, void* result, socklen_t* len);  //获取socket的option信息
    template<class T>
    ...
    int send(const iovec* buffer, size_t length, int flag = 0);
    ...
    std::ostream& dump(std::ostream& os) const; //格式化输出socket信息
    int getSocket() const { //获取socket句柄 
        return m_sock;
    }
