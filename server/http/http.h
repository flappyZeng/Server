...
//判断是不是能够转换成功,存在的话就转换键值对应的值为T并赋值给val
...
public:
    ...
    HttpStatues getStatus() const {return m_status;}
    uint8_t getVersion() const {return m_version;}
    const std::string& getBody() const {return m_body;}
    ...
    void setStatus(HttpStatues v) {m_status = v;}
    void setVersion(uint8_t v) { m_version = v;}
    void setBody(const std::string& v) { m_body = v;}
    ...
    std::string toString() const;
private:
    HttpStatues m_status;
    uint8_t m_version;
    bool m_close;

    std::string m_body;
    std::string m_reason;  //响应码对应的原因
    MapType m_headers;
